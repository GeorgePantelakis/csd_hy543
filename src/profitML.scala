 ...
 *
 * profitML.scala
 * Training and evaluating process for profit linear regression model
 */
...
def labeledPointTransform(entry: Entry): LabeledPoint = {
	// profit = fare_amount - (trip_distance * 0.15) + tip_amount
	val profit = entry.fare_amount.toDouble - (entry.trip_distance.toDouble * 0.15) + entry.tip_amount.toDouble;
	val dayOfWeek = entry.pickup_datetime.getDayOfWeek.getValue.toDouble;
	val hourOfDay = entry.pickup_datetime.getHour.toDouble;
	val minuteOfDay = entry.pickup_datetime.getMinute.toDouble;
	LabeledPoint(profit,
		Vectors.dense(dayOfWeek,
			      hourOfDay,
			      minuteOfDay,
...
bestModel.asInstanceOf[LinearRegressionModel].save("./profit.model")