 ...
 *
 * tipAmountML.scala
 * Training and evaluating process for tip amount linear regression model
 */
...
def labeledPointTransform(entry: Entry): LabeledPoint = {
	...
	LabeledPoint(entry.tip_amount.toDouble,
		Vectors.dense(total_time.toDouble,
			      ...
			      entry.payment_type.toDouble,
			      entry.fare_amount.toDouble))
}
...
bestModel.asInstanceOf[LinearRegressionModel].save("./tipAmount.model")