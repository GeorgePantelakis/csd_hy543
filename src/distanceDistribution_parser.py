...
for distanceDistributionDir in glob(f'./plotData/distanceDistribution*.data'):
	for part in glob(f'{distanceDistributionDir}/part-*'):
			...
			with open(part, 'r') as partfp:
				for line in partfp:
					if line != '\n':
						...
						parsed_data[int(fields[0])][fields[1]] += float(fields[2])

for year, month_data in parsed_data.items():
	with open(f'./plotDataNew/distanceDistributionYear{year}.dat', 'w') as fp:
